    ...
    constructor(args = {}) {
        ...
        */
        ...
        this.sendTextToGroup = async (remoteJid, message, messages = null) => {
            try {
                // Format the group ID (if needed)
                const groupId = utils_1.default.formatPhone(remoteJid);

                // Send the text message to the group
                await this.vendor.sendMessage(groupId, {
                    text: message
                }, { quoted: messages });

                return { status: 'success', message: 'Message sent to group successfully' };
            } catch (error) {
                console.error(`Error sending text to group: ${error}`);
                throw error;
            }
        };
        this.vendor = null;
